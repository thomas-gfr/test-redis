...
try {
    ...
    class UserManager
    {
        ...
        public function createUser($firstName, $lastName, $age, $job, $email, $password)
        {
            $uuid = Uuid::uuid4()->toString();

            $hashedPassword = password_hash($password, PASSWORD_BCRYPT);

            $stmt = $this->pdo->prepare("INSERT INTO users ( password, firstName, lastName, age, job, email) VALUES ( ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$hashedPassword, $firstName, $lastName, $age, $job, $email]);
        ...
        public function updateUser($uuid, $firstName, $lastName, $age, $job, $email)
        {
            // $stmt = $this->pdo->prepare("UPDATE users SET username=?, sport=?, tel=?, age=? WHERE id=?");
            $stmt = $this->pdo->prepare("UPDATE users SET firstName=?, lastName=?, age=?, job=?, email=? WHERE id=?");
            // $stmt->execute([$username, $sport, $tel, $age, $uuid]);
            $stmt->execute([$firstName, $lastName, $age, $job, $email, $uuid]);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($firstName) || empty($lastName) || empty($age) || empty($job) || empty($email) || empty($password)) {
        ...
        // Crée l'utilisateur
        $userManager->createUser($firstName, $lastName, $age, $job, $email, $password);
        ...
        if (
            ...
            $userManager->updateUser($uuid, $firstName, $lastName, $age, $job, $email);
